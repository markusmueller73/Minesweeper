...
use crate::minesweeper::cell::{Cell, CellMarker};
...
impl Default for Board {
    fn default() -> Self {
        Board::new(BoardSize::Small, false)
    }
}
...
impl Board {
    ...
    /// Clear all cells of the game board and create a new board with the given size
    pub fn set_new_board(&mut self, board_size: BoardSize) {
        self.clear();
        let (w,h) = match board_size {
            BoardSize::Small => (MAX_BOARD_WIDTH-20,MAX_BOARD_HEIGHT-20),
            BoardSize::Medium => (MAX_BOARD_WIDTH-10,MAX_BOARD_HEIGHT-10),
    ...
    /// Get the cell condition
    pub fn get_cell(&self, x: usize, y: usize) -> (CellMarker, bool, u8, bool) {
        if self.is_cell_valid(x as isize, y as isize) {
            let marker = self.cell[x][y].get_state();
            let is_revealed = self.cell[x][y].is_revealed();
            let bombs_around = self.cell[x][y].get_bombs_around();
            let is_bomb = self.cell[x][y].is_bomb();
            return (marker, is_revealed, bombs_around, is_bomb);
        }
        (CellMarker::None, false, 0, false)
    }
    ...
    pub fn check_correct_flagged_bombs(&self) -> u32 {
