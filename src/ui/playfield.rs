use crate::game::{WINDOW_HEIGHT, WINDOW_WIDTH, GameState};
use crate::minesweeper::cell::CellMarker;
use crate::ui::{MENU_BUTTON_HEIGHT, MENU_BUTTON_WIDTH};
use egor::{
    math::vec2,
...
pub fn draw_playfield(graphics: &mut Graphics, game_state: &GameState) {
    ...
    let x_offset = ((WINDOW_WIDTH as f32) - cell_size * cols as f32 ) / 2.0;
    let y_offset = (WINDOW_HEIGHT as f32) - cell_size * rows as f32 - 10.0;
    ...
    // Draw game title
    graphics.rect()
        .at(vec2((WINDOW_WIDTH as f32 - MENU_BUTTON_WIDTH) / 2., 0.0))
        .size(vec2(MENU_BUTTON_WIDTH, MENU_BUTTON_HEIGHT))
        .texture(*game_state.texture.get("logo").unwrap() as usize);
    ...
    graphics.rect()
        .at(vec2(WINDOW_WIDTH as f32 - x_offset - 75.0, y_offset - 50.0))
        .size(vec2(20.0, 45.0))
        .texture(time_tex1);
    graphics.rect()
